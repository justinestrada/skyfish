        ...
        @foreach ($faqs['qa'] as $key => $qa)
          <div class="bg-{{ $background['color'] }} text-{{ $text_color }} accordion-item" data-question="{{ $qa['question'] }}"
            ...
            <h2 class="accordion-header" id="qa-heading-{{ $key }}">
              <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#qa-collapse-{{ $key }}" aria-expanded="false"
                aria-controls="qa-collapse-{{ $key }}">
                {{ $qa['question'] }}
              </button>
            </h2>
            <div id="qa-collapse-{{ $key }}" class="accordion-collapse collapse text-left" aria-labelledby="qa-heading-{{ $key }}" data-bs-parent="#qaAccordion">
